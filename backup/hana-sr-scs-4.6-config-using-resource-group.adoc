...
HANA 시스템 데이터베이스에 대해 백업 및 복구 작업을 실행하는 데 필요한 권한을 가진 데이터베이스 사용자 "SnapCenter"가 생성되었습니다(참조). link:hana-sc-generic-scope.html["SnapCenter를 사용한 SAP HANA 백업 및 복구"^]). 위의 데이터베이스 사용자를 사용하여 두 호스트 모두에 HANA 사용자 저장소 키를 구성해야 합니다.
...
SnapCenter 4.6 리소스 그룹 구성에 대한 복원 및 복구 작업은 자동 검색된 비시스템 복제 설정과 동일합니다. 모든 복원 및 자동 복구 옵션을 사용할 수 있습니다. 자세한 내용은 기술 보고서를 참조하십시오. link:hana-sc-generic-scope.html["SnapCenter를 사용한 SAP HANA 백업 및 복구"^].
