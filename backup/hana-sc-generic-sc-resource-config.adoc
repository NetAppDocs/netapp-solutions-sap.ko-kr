...
이전 항목에서 설명한 대로 초기 SnapCenter 구성 link:hana-sc-generic-sc-initial-config.html["SnapCenter 초기 구성"] HANA 데이터베이스 리소스를 구성하는 동안 자격 증명, 스토리지 시스템 및 정책이 필요하므로 먼저 이 작업을 수행해야 합니다. 아래 그림은 단계와 종속성을 요약한 것입니다.
