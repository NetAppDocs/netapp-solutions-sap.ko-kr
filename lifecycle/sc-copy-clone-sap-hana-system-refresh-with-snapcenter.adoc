...
이후의 모든 단계는 " 섹션에 설명된 대로 운영 스토리지에서 클론을 생성하는 워크플로와 동일합니다<<Cloning from primary storage with tenant name equal to SID>>.”
...
SnapCenter의 필수 단계는 " 섹션에 설명된 단계와 동일합니다<<Cloning from primary storage with tenant name equal to SID>>.”] 차이점은 스크립트 'c-system-refresh.sh'의 테넌트 복구 작업에 있습니다.
