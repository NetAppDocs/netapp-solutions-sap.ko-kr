...
일반적으로 복구 시스템과 재해 복구 테스트 시스템은 SAP 시스템 클론(광범위한 후처리 작업이 필요하지 않음)을 사용하여 새로 고쳐진 테스트 및 교육 시스템에 제공되지만, 소스 시스템과의 공존을 가능하게 하려면 이러한 후처리 단계를 적용해야 합니다. 따라서 이 문서에서는 SAP 시스템 새로 고침 시나리오에 중점을 둡니다. 다양한 사용 사례에 대한 자세한 내용은 기술 보고서 link:sc-copy-clone-introduction.html["SnapCenter를 사용하여 SAP HANA 시스템 복사 및 클론 작업 자동화"^]를 참조하십시오.
