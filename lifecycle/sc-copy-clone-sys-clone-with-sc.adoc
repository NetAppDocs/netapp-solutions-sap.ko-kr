...
+
시스템이 더 이상 필요하지 않은 경우, 다음 단계에 따라 정리 프로세스를 수행합니다.

+
.. 데이터베이스 종료를 포함하여 SAP HANA 데이터 볼륨에 대한 SnapCenter 클론 삭제 워크플로를 수행합니다.
+
[NOTE]
====
SnapCenter 클론 삭제 워크플로는 기본적으로  `/etc/fstab` 항목을 삭제하지 않습니다.

 `/etc/fstab` 항목이 클론 삭제 과정에서 자동으로 삭제되려면 먼저 클론을 복구하고 자동 검색 리소스로 설정해야 합니다. 이 단계를 수행하지 않으면  `/etc/fstab` 항목이 자동으로 삭제되지 않으므로 수동으로 제거해야 합니다.

====
.. SAP HANA 서비스 중지
.. SAP HANA 공유 볼륨에 대한 SnapCenter 클론 삭제 워크플로를 수행합니다.
