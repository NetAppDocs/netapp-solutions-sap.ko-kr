...
SAP HANA 영구 암호화가 활성화되고 암호화 루트 키에 사용되는 보안 저장소(인스턴스 SFSS 또는 로컬 보안 저장소 LSS)에 따라 대상 시스템에서 SAP HANA 데이터베이스를 복구하기 전에 추가 단계가 필요합니다.

SFSS에서 루트 키는 파일 시스템에 저장되므로 HANA 데이터베이스와 별도로 백업해야 합니다. 백업된 루트 키는 대상 시스템으로 가져와야 HANA 데이터베이스 복구가 가능해집니다. 루트 키가 변경되지 않는 경우 이 작업은 한 번만 수행하면 됩니다.

LSS를 사용하면 루트 키가 HANA 데이터베이스와 함께 단일 작업으로 백업 및 복원됩니다. HANA 데이터베이스를 복구하려면 루트 키 백업에 사용된 암호가 필요합니다.

필요한 작업은 다음 장에서 더 자세히 설명합니다.
...
NOTE: 최소 HANA 버전 2.00.088을 사용해야 합니다. 그렇지 않으면 설명된 절차가 작동하지 않습니다. SAP note 3613110도 참조하십시오.

암호화 및 LSS를 사용하는 HANA 시스템을 복구하려면 루트 키를 백업하는 데 사용된 암호가 필요합니다.
...
....
대상 시스템의 LSS 구성에서 소스 시스템의 암호를 설정하는 데 사용할 수 있습니다. 시스템 데이터베이스와 테넌트 데이터베이스에 동일한 명령을 사용해야 합니다.

사전에 암호를 설정하지 않은 경우 복구 작업 중에 암호를 입력할 수 있습니다.
