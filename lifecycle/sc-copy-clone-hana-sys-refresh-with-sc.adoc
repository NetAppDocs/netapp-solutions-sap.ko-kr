...
|  | SAP HANA 지속성 암호화 | SFSS: 초기 설정 시 또는 소스 시스템에서 루트 키가 변경된 경우, 복구를 실행하기 전에 대상 시스템에 루트 키 백업을 가져와야 합니다. LSS: 초기 설정 시 또는 루트 키 백업 암호가 변경된 경우, LSS 구성에 암호를 설정해야 합니다. 
...
|  | SAP HANA 지속성 암호화 | SFSS: 소스 시스템의 DBID가 대상 시스템에 이미 존재하지 않는 경우, 해당 테넌트의 루트 키 백업을 가져오기 전에 먼저 시스템 데이터베이스를 복구해야 합니다. LSS: 테넌트 데이터베이스 복구 중에 LSS 토폴로지가 HANA 토폴로지와 자동으로 동기화됩니다. 
...
모든 SAP HANA 시스템은 구성 가이드에 따라 구성해야 합니다. link:../bp/hana-aff-nfs-introduction.html["NFS를 포함한 NetApp AFF 시스템 기반 SAP HANA"]. SnapCenter 와 SAP HANA 리소스는 모범 사례 가이드에 따라 구성되었습니다. link:../backup/hana-sc-generic-scope.html["SnapCenter를 사용한 SAP HANA 백업 및 복구"].
...
. SAP HANA 대상 시스템 설치
. SnapCenter 에서 SAP HANA 시스템을 구성하는 방법은 다음과 같습니다. link:../backup/hana-sc-generic-scope.html["SnapCenter를 사용한 SAP HANA 백업 및 복구"]
+
...
파이버 채널 SAN 설정에서는 다음 화면에서 사용된 프로토콜을 제공해야 합니다.
