...
| 스토리지 프로토콜 및 파일 시스템 | 베어 메탈: NFS, XFS를 사용한 FC VMware: 게스트 내 NFS 마운트, VMDK 및 XFS를 사용한 FC | NFS 를 참조하십시오 | NFS 를 참조하십시오 


| 씬 클론(FlexClone) | 예 | 예 | 예, 하지만 SnapCenter에 통합되지는 않습니다 


| 클론 분할 작업 | 예 | 예 | 예, 하지만 SnapCenter에 통합되지는 않습니다 


| 운영 사이트에서 클론 복제 | 예 | 예 | 예 


| 오프 사이트 백업에서 복제 | 예 | 예 | 예, ANF 백업에서 새 볼륨으로 복원합니다 


| DR 사이트에 복제 | 예 | 예 | 예, 하지만 SnapCenter에 통합되지는 않습니다 
|===
VMware VMFS 및 NetApp ASAr2 시스템을 사용하는 HANA용 SAP 시스템 새로 고침 작업에 대한 추가 정보는  https://docs.netapp.com/us-en/netapp-solutions-sap/backup/hana-sc-vmfs-asar2sap-refresh.html["SAP 시스템 새로 고침"]에서 확인할 수 있습니다.
