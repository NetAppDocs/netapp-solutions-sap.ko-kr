...
다음 표는 단일 호스트 SAP HANA 시스템의 LUN 구성을 보여줍니다.

|===
| 목적 | LUN | LVM 논리 볼륨 


| 데이터 | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| 로그 | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| 공유됨 | FC5_shared | 해당 없음 
|===

NOTE: 위에서 설명한 구성에서는 사용자 SID1adm의 기본 홈 디렉토리가 저장되는 `/usr/sap/SID1` 디렉토리가 로컬 디스크에 있습니다. 디스크 기반 복제를 사용하는 재해 복구 설정에서 NetApp은 모든 파일 시스템이 중앙 스토리지에 있도록 `/usr/sap/SID1` 디렉토리용 LUN을 추가로 생성할 것을 권장합니다.


NOTE: SAP 로컬 보안 저장소 서비스(LSS)를 사용하는 경우 LSS의 기본 설치 디렉터리인  `/lss/shared/SID` 디렉터리는 로컬 디스크에 있습니다. 디스크 기반 복제를 사용하는 재해 복구 환경에서 NetApp은 모든 파일 시스템이 중앙 스토리지에 있도록  `/lss/shared/SID` 디렉터리용 추가 LUN을 생성하는 것을 권장합니다.
