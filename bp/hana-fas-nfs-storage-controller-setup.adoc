...
ONTAP Autonomous Ransomware Protection으로 생성된 스냅샷은 SAP HANA 데이터베이스의 순방향 복구에 사용할 수 없습니다. 따라서 SAP HANA 데이터 및 로그 볼륨에는 이 기능을 사용하지 않는 것이 좋으며, 해당 볼륨에서는 이 기능을 비활성화해야 합니다. 랜섬웨어 보호를 위해서는 대신 SnapCenter 기반 데이터 보호 기능과 변조 방지 스냅샷을 사용하십시오: link:../backup/hana-sc-generic-scope.html["SnapCenter를 사용한 SAP HANA 백업 및 복구"]. ONTAP Autonomous Ransomware Protection은 /hana/shared와 같은 다른 볼륨에는 유용할 수 있습니다.
...
NOTE: SAP 로컬 보안 저장소 서비스(LSS)를 사용하는 경우 `/lss/shared/SID` 디렉터리, LSS의 기본 설치 디렉터리는 로컬 디스크에 있습니다. 디스크 기반 복제를 사용하는 재해 복구 환경에서 NetApp은 모든 파일 시스템이 중앙 스토리지에 있도록 `SID_shared` 볼륨 내에 `/lss/shared/SID` 디렉터리를 생성하는 것을 권장합니다.
...
| SID_공유됨 | lss-shared | /lss/shared/ | 모든 호스트에 마운트되었습니다 
