...
. "에 설명된 대로 디스크 쉘프 구성이 올바른지 확인합니다. shelf connection있습니다."
. 에 설명된 대로 필요한 애그리게이트를 생성하고 구성합니다. configuration있습니다."
. "Clustered Data ONTAP 8.3.1에서 설명하는 대로 스토리지 가상 시스템(SVM)을 configuration있습니다."
. "에 설명된 대로 LIF 생성 configuration있습니다."
. ""에 설명된 대로 애그리게이트 내에 볼륨을 생성합니다. configuration for SAP HANA single host systems"및" configuration for SAP HANA multiple host systems있습니다."
. 에 설명된 대로 필요한 볼륨 옵션을 설정합니다. options있습니다."
. 의 설명대로 NFSv3에 필요한 옵션 설정 configuration for NFSv3"또는 NFSv4의 경우 에 설명된 대로 configuration for NFSv4있습니다."
. 볼륨을 네임스페이스에 마운트하고 " 에 설명된 대로 엑스포트 정책을 설정합니다. volumes to namespace and set export policies있습니다."
...
SAP HANA 다중 호스트 시스템이 사용되는 경우 다음 표에 나와 있는 것처럼 SVM에서 NFSv4 임대 시간을 설정합니다.
...
|===
HANA 2.0 SPS4부터 HANA는 페일오버 동작을 제어하는 매개 변수를 제공합니다. SVM 레벨에서 임대 시간을 설정하는 대신 이러한 HANA 매개 변수를 사용하는 것이 좋습니다.

파라미터는 다음 표와 같이 'nameserver.ini` 내에 있다. 이 섹션 내에서 기본 재시도 간격을 10초로 유지합니다.
