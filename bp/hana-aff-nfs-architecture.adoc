...
NOTE: SAP HANA 백업 및 복구 솔루션에 대한 자세한 내용은 다음을 참조하십시오. link:../backup/hana-sc-generic-scope.html["SnapCenter를 사용한 SAP HANA 백업 및 복구"^].
...
image:saphana_aff_nfs_image7b.png["NetApp SnapMirror 를 참조하십시오"]
...
NetApp SnapMirror NAS용 액티브 동기화는 NFSv4.1을 통해 SAP HANA와 함께 사용되며, 두 개의 ONTAP 클러스터 간 SVM 미러링을 기반으로 합니다. 이 아키텍처는 NAS 데이터와 관련 SVM 구성을 동기적으로 복제하여 미션 크리티컬 SAP HANA 워크로드에 지속적인 가용성과 제로 RPO 보호를 제공합니다.

SVM 복제를 사용하면 볼륨, 논리 인터페이스, 내보내기 정책, NFS 설정 및 액세스 구성 등 주요 NAS 구성 요소가 보조 사이트로 미러링됩니다. 이를 통해 운영 복잡성이 줄어들고 기본 사이트 또는 스토리지 시스템을 사용할 수 없게 될 경우 보조 환경이 신속하게 인계받을 수 있습니다.

ONTAP Mediator는 참여하는 ONTAP 시스템을 모니터링하고 장애 발생 시 쿼럼 결정을 지원합니다. 이를 통해 스플릿 브레인 현상을 방지하고 제어된 페일오버 동작을 구현할 수 있습니다. SnapMirror Active Sync for NAS와 ONTAP Mediator는 함께 고가용성, 동기식 복제 및 강력한 복구 목표가 요구되는 SAP HANA 환경을 위한 안정적인 스토리지 기반을 제공합니다.

image:saphana_aff_nfs_image6.png["NetApp SnapMirror 활성 동기화"]
