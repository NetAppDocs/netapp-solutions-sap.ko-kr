...
....
주어진 LUN에 대해 최대 처리량으로 1000MB/s를 사용하는 것이 좋습니다.  애플리케이션에 더 많은 처리량이 필요한 경우 LUN 스트라이핑을 사용한 여러 LUN을 사용하여 필요한 대역폭을 제공해야 합니다.  이 가이드에서는 Linux LVM 기반 SAP HANA에 대한 예를 섹션에 제공합니다.link:hana-fas-fc-host-setup.html["호스트 설정"] .
...
. 설명된 대로 올바른 디스크 선반 구성을 확인하십시오.<<디스크 쉘프 연결>> .
