...
NOTE: 다음 예는 LSS를 사용하는 SID=NF2인 SAP HANA 데이터베이스를 보여줍니다.
...
다음 예제는 SID=NF2인 다중 호스트 SAP HANA 데이터베이스를 보여줍니다. 데이터 및 로그 파일 시스템에는 NFSv4.1을, `/hana/shared`, `/lss/shared`, 및 `/usr/sap/NF2` 파일 시스템에는 NFSv3을 사용합니다. NFS 전송 크기는 읽기의 경우 1MB, 쓰기의 경우 256k가 사용됩니다.
