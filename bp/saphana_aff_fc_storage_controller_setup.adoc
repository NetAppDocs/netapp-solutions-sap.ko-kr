...
. "Clustered Data ONTAP 8.3.1에서 설명하는 대로 스토리지 가상 시스템(SVM)을 virtual machine configuration있습니다."
. "에 설명된 대로 논리 인터페이스(LIF) 생성 interface configuration있습니다."
. ""에 설명된 대로 포트 세트를 생성합니다. port sets있습니다."
. 생성 에 설명된 대로 애그리게이트 내에 이니시에이터 그룹, 볼륨 및 LUN을 생성합니다. and volumes and mapping LUNs to initiator groups있습니다."
