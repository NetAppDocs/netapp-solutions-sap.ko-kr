...
NOTE: SAP 로컬 보안 저장소 서비스(LSS)를 사용하는 경우 `/lss/shared/SID` 디렉터리, LSS의 기본 설치 디렉터리는 로컬 디스크에 있습니다. 디스크 기반 복제를 사용하는 재해 복구 환경에서 NetApp은 모든 파일 시스템이 중앙 스토리지에 있도록 `SID1_shared` 볼륨 내에 `/lss/shared/SID` 디렉터리용 LUN을 추가로 생성하는 것을 권장합니다.
