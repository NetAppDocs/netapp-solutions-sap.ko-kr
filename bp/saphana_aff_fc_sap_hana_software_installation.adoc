...
....
+
Linux LVM을 사용하는 경우 필요한 구성은 다릅니다. 다음 예에서는 SID = FC5인 2+1 다중 호스트 설정을 보여 줍니다.
...
NOTE: 준비된 global.ini` 파일이 저장된 디렉토리는 'storage_cfg' CLI 옵션('--storage_cfg=/hana/shared')에 포함되어 있습니다.
...
link:saphana_aff_fc_adding_additional_data_volume_partitions_for_sap_hana_single-host_systems.html["다음: SAP HANA 단일 호스트 시스템을 위한 추가 데이터 볼륨 파티션 추가."]
