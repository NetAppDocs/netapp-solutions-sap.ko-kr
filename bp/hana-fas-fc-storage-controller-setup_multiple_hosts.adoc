...
다음 그림에서는 4+1 다중 호스트 SAP HANA 시스템의 볼륨 구성을 보여 줍니다. 각 SAP HANA 호스트의 데이터 볼륨 및 로그 볼륨은 서로 다른 스토리지 컨트롤러에 분산됩니다. 예를 들어, 볼륨의 ID_DATA_mnt00001은 컨트롤러 A에 구성되고 볼륨의 ID_LOG_mnt00001은 컨트롤러 B에 구성됩니다 각 볼륨 내에 하나의 LUN이 구성됩니다.
 ...
 `/hana/shared` 볼륨은 모든 HANA 호스트에서 액세스할 수 있어야 하므로 NFS를 사용하여 내보냅니다.  `/hana/shared` 파일 시스템에 대한 특정 성능 KPI는 없지만 NetApp에서는 10Gb 이더넷 연결을 사용할 것을 권장합니다.


NOTE: SAP HANA 시스템에 HA 쌍의 스토리지 컨트롤러를 하나만 사용하는 경우, 데이터 및 로그 볼륨을 동일한 스토리지 컨트롤러에 저장할 수 있습니다.

image:saphana_fas_fc_image19.png["입력/출력 대화 상자 또는 작성된 내용을 표시하는 그림"]
...
NOTE: SAP 로컬 보안 저장소 서비스(LSS)를 사용하는 경우  `/lss/shared/SID` 디렉터리는 모든 호스트에서 액세스할 수 있어야 합니다. NetApp에서는  `/lss/shared/SID` 디렉터리용 볼륨을 생성하고 NFS를 사용하여 모든 호스트에 마운트하는 것을 권장합니다.
...
LVM 볼륨 그룹의 LUN은 서로 다른 애그리게이트 및 서로 다른 컨트롤러에 저장되어야 합니다. 다음 표는 2+1 SAP HANA 다중 호스트 시스템에서 볼륨 그룹당 두 개의 LUN을 사용하는 예시를 보여줍니다.


NOTE: SAP HANA KPI를 충족하기 위해 여러 LUN을 결합하는 데 LVM을 사용할 필요는 없지만 권장됩니다.
...
NOTE: 아래 예에서는 볼륨 생성 중에 이러한 옵션을 설정합니다
