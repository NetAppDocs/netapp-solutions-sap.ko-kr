...
NOTE: SAP HANA 다중 호스트 시스템에서는  `/hana/shared` 파일 시스템이 필요하며, SAP Local Secure Store 서비스(LSS)를 사용하는 경우  `/lss/shared` 파일 시스템이 시스템의 모든 호스트에 연결되어야 합니다. 일반적으로 이는 NFS 서버에서 제공하는 NFS 공유입니다. NetApp FAS 또는 AFF 시스템과 같은 고가용성 NFS 서버를 사용하는 것이 좋습니다. 또 다른 옵션은 Linux 호스트에 내장된 NFS 서버를 사용하는 것입니다.
...
파일 시스템을 마운트하려면 다음을 실행하세요. `mount -a` 각 호스트에서 명령을 내립니다.
