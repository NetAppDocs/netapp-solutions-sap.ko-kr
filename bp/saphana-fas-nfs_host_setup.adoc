...
NOTE: nconnect가 NFSv4.x와 함께 사용되는 경우 다음 규칙에 따라 NFSv4.x 세션 슬롯의 양을 조정해야 합니다.
...
그런 다음 다시 부팅합니다. 서버 측 값도 조정되어야 하며 에 설명된 대로 세션 슬롯 수를 설정해야 합니다 link:saphana-fas-nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["NFSv4에 대한 NFS 구성."]
