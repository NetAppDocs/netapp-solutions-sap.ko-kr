...
`echo options nfs max_session_slots=<calculated value> > /etc/modprobe.d/nfsclient.conf` 후 종료됩니다. 서버 측 값도 조정되어야 하며 에 설명된 대로 세션 슬롯 수를 설정해야 합니다 link:saphana-fas-nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["NFSv4에 대한 NFS 구성."]
