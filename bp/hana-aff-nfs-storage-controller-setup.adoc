...
ONTAP Autonomous Ransomware Protection으로 생성된 스냅샷은 SAP HANA 데이터베이스의 정방향 복구에 사용할 수 없습니다. 따라서 SAP HANA 데이터 및 로그 볼륨에는 사용하지 않는 것이 좋으며 이러한 볼륨에 대해 비활성화해야 합니다. 랜섬웨어 보호를 위해 변조 방지 스냅샷이 포함된 SnapCenter 기반 데이터 보호를 대신 사용하십시오: link:../backup/hana-sc-generic-scope.html["SnapCenter를 사용한 SAP HANA 백업 및 복구"]. ONTAP Autonomous Ransomware Protection은 /hana/shared와 같은 다른 볼륨에 유용할 수 있습니다.

ONTAP 클러스터의 볼륨 생성 시 이 기능의 자동 활성화를 비활성화할 수 있지만, 기본 설정을 유지하고 이 기능이 필요하지 않은 볼륨에 대해서만 비활성화하는 것이 좋습니다.
...
image:saphana_aff_nfs_image13.png["SAS 쉘프 연결"]



=== NVMe(100GbE) 디스크 쉘프

각 NS224 NVMe 디스크 쉘프는 다음 그림과 같이 컨트롤러당 2개의 100GbE 포트를 통해 연결됩니다. 각 쉘프 내의 디스크는 HA 쌍의 두 컨트롤러에 균등하게 분산되어야 합니다. 애그리게이트 구성 장에서 설명한 ADPv2도 NS224 디스크 쉘프에 사용됩니다.

image:saphana_aff_nfs_image14a.png["MVMe 디스크 쉘프 연결"]



== 애그리게이트 구성

일반적으로 사용되는 디스크 쉘프 또는 드라이브 기술(SAS SSD 또는 NVMe SSD)에 관계없이 컨트롤러당 2개의 애그리게이트를 구성해야 합니다.
...
image:saphana_aff_nfs_image15a.png["애그리게이트 구성"]
...
image:saphana_aff_nfs_image16a.png["볼륨 구성 SAP HANA 단일 호스트"]
...
NOTE: SAP 로컬 보안 저장소 서비스(LSS)를 사용하는 경우 `/lss/shared/SID` 디렉터리(LSS의 기본 설치 디렉터리)는 로컬 디스크에 있습니다. 디스크 기반 복제를 사용하는 재해 복구 환경에서 NetApp은 모든 파일 시스템이 중앙 스토리지에 있도록 `SID_shared` 볼륨 내에 `/lss/shared/SID` 디렉터리를 생성하는 것을 권장합니다.
...
| SID 공유 | usr-sap + shared + lss-shared | /usr/sap/SID + /hana/shared + /lss/shared/SID 
|===
...
image:saphana_aff_nfs_image17a.png["볼륨 구성 SAP HANA 다중 호스트"]
...
NOTE: SAP 로컬 보안 저장소 서비스(LSS)를 사용하는 경우  `/lss/shared/SID` 디렉터리는 모든 호스트에서 액세스할 수 있어야 합니다. NetApp에서는  `SID_shared` 볼륨 내에  `/lss/shared/SID` 디렉터리용 디렉터리를 생성하는 것을 권장합니다.
...
| SID_공유됨 | lss-shared | /lss/shared/SID | 모든 호스트에 마운트되었습니다 
...
| SID_shared 볼륨을 제외하고 랜섬웨어 보호를 비활성화합니다 | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state disabled 
...
NOTE: 숫자 ID를 비활성화하려면 섹션에 설명된 대로 사용자 관리가 필요합니다 link:hana-aff-nfs-install-prep-nfsv4.html["“NFSv4를 위한 SAP HANA 설치 준비”"]


NOTE: (`/etc/idmapd.conf`섹션 에 설명된 대로 NFSv4 도메인 ID는 모든 Linux 서버 및 SVM에서 동일한 값으로 설정해야 합니다 link:hana-aff-nfs-install-prep-nfsv4.html["“NFSv4를 위한 SAP HANA 설치 준비”"]
